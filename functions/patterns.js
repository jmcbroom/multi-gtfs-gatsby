...
exports.handler = async function(event) {
  ...
  const urls = {
    ...
    umich: `https://mbus.ltp.umich.edu/bustime/api/v3/getpatterns?key=${process.env.UMICH_KEY}&format=json&rt=${event.queryStringParameters.routeId}`,
  ...
  try {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ message: 'Error fetching patterns', error: error.message }),
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': corsOrigin
      }
